...
const PostEditorModal: React.FC<PostEditorModalProps> = ({ post, onClose }) => {
  const isEditing = post !== null;
  const { addBlogPost, updateBlogPost } = useContext(SettingsContext);
  const { apiKey } = useContext(AiServiceContext);

  const [formData, setFormData] = useState<Omit<BlogPost, 'id' | 'publishedDate'>>({
    title: post?.title || '',
    slug: post?.slug || '',
    content: post?.content || '',
    author: post?.author || 'Admin',
    status: post?.status || 'draft',
    seo: post?.seo || { title: '', description: '' },
  });
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      title: newTitle,
      slug: slugify(newTitle), // Auto-update slug
    }));
  };
  ...
  const handleGenerateContent = async () => {
      ...
      const res = await generateBlogPost(apiKey, formData.title);
  ...
  const handleGenerateTitle = async () => {
    if (!apiKey || !formData.content) return;
    setIsGenerating(true);
    const prompt = `Generate a short, catchy blog post title based on the following content:\n\n${formData.content.substring(0, 500)}...`;
    const res = await generateText(apiKey, prompt);
    if(res.success && res.text) {
        handleTitleChange({ target: { value: res.text.replace(/"/g, '') } } as React.ChangeEvent<HTMLInputElement>);
    }
    setIsGenerating(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isEditing && post) {
      await updateBlogPost({ ...formData, id: post.id, publishedDate: post.publishedDate });
    } else {
      // FIX: Removed 'publishedDate' as it is not an expected property for 'addBlogPost'.
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-base-100-light dark:bg-base-200-dark w-full max-w-4xl h-[90vh] rounded-xl shadow-2xl border border-base-300-light dark:border-base-300-dark flex flex-col animate-fade-in" onClick={e => e.stopPropagation()}>
        <header className="flex items-center justify-between p-4 border-b border-base-300-light dark:border-base-300-dark">
          <h2 className="text-xl font-bold">{isEditing ? 'Edit Blog Post' : 'Add New Blog Post'}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 p-6 space-y-4 overflow-y-auto">
             <div className="flex items-end gap-2">
                ...
                <button type="button" onClick={handleGenerateTitle} disabled={!apiKey || !formData.content || isGenerating} className="px-3 py-2 bg-amber-500 text-white rounded-md text-sm disabled:bg-gray-400">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="author" value={formData.author} onChange={handleChange} className="w-full p-2 bg-base-200-light dark:bg-base-300-dark rounded-md border border-gray-300 dark:border-gray-600" />
              </div>
              <div>
